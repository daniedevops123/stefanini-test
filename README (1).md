...
Sí, se utilizó IA (Claude de Anthropic) como asistente durante el desarrollo de esta prueba técnica. A continuación, el detalle de en qué partes:

| Parte | Uso de IA |
|---|---|
| Estructura general del pipeline | Claude propuso la arquitectura de 4 stages y la lógica de condicionales (`if: github.ref == 'refs/heads/main'`) |
| Smoke test en Python | Claude generó los assertions de validación de los endpoints dentro del runner de GitHub Actions |
| Terraform template | Claude redactó el `main.tf` con variables, locals y outputs documentados |
| README | Claude estructuró y redactó el README completo en español |
| Dockerfile | Revisado y ajustado con sugerencias de Claude (HEALTHCHECK, Alpine, instalación global de json-server) |

La lógica de orquestación, la elección de herramientas, la estrategia de persistencia y las decisiones de arquitectura fueron definidas y validadas por el desarrollador. La IA actuó como par de programación y redactor técnico, no como autor autónomo.
